...
async def diary_get_text(message: Message, state: FSMContext):
    if len(message.text) > 2000:
        await message.answer("Текст слишком длинный (макс. 2000 символов).")
        return
...
async def diary_date_other(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text("Введите дату в формате <b>ДД.ММ.ГГГГ</b>:")
    await state.set_state(DiaryForm.entry_date)
    await callback.answer()
...
async def diary_set_date(message: Message, state: FSMContext):
    try:
        entry_date = datetime.strptime(message.text, "%d.%m.%Y").date()
...
async def diary_list(callback: CallbackQuery, page: int = 0):
    async with async_session() as session:
        ...
        if not user:
            ...
            return
        total = await session.scalar(select(func.count(DiaryEntry.id)).where(DiaryEntry.user_id == user.id))
        if total == 0:
        ...
        for e in entries_list:
            ...
            text += f"{emotion_emoji} <b>{e.date.strftime('%d.%m.%Y')}</b>\n{e.text[:100]}{'...' if len(e.text)>100 else ''}\n"
            if e.tags:
                text += f"🏷️ {e.tags}\n"
...
async def edit_entry_start(message: Message, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        if not entry:
            ...
            return
        user = await session.scalar(select(User).where(User.tg_id == message.from_user.id))
        if not user or entry.user_id != user.id:
...
async def search_by_date_result(message: Message, state: FSMContext):
    try:
        search_date = datetime.strptime(message.text, "%d.%m.%Y").date()
    except:
    ...
    async with async_session() as session:
        ...
        if not user:
            ...
            return
        entries = await session.execute(
            select(DiaryEntry).where(DiaryEntry.user_id == user.id, DiaryEntry.date == search_date)
            .order_by(DiaryEntry.date.desc())
        )
        entries_list = entries.scalars().all()
        if not entries_list:
            ...
            text = f"📅 Записи за {search_date.strftime('%d.%m.%Y')}:\n\n"
            for e in entries_list:
                emotion_emoji = next((emoji for emoji, code in EMOTIONS.items() if code == e.emotion), "📌")
...
async def search_by_tag_result(message: Message, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        if not user:
            ...
            return
        entries = await session.execute(
            select(DiaryEntry).where(DiaryEntry.user_id == user.id, DiaryEntry.tags.contains(tag))
            .order_by(DiaryEntry.date.desc())
        )
        entries_list = entries.scalars().all()
        if not entries_list:
