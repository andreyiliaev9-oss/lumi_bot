...
from config import LEVEL_THRESHOLDS, settings
...
class Habit(Base):
    ...
    reminder_days = Column(String(20))  # JSON: [1,2,3,4,5,6,7]
...
async def get_session() -> AsyncGenerator[AsyncSession, None]:
    """Получение сессии БД (для DI/depends)"""
...
async def complete_habit(habit_id: int, status: str = "completed") -> None:
...
async def get_or_create_cycle_tracker(user_id: int) -> CycleTracker:
    async with async_session() as session:
        result = await session.execute(select(CycleTracker).where(CycleTracker.user_id == user_id))
...
async def update_cycle_tracker(
    user_id: int,
...
# Admin operations
async def add_admin_message(message_type: str, content: str, scheduled_time: str) -> None:
    async with async_session() as session:
        msg = AdminMessage(
    ...
    async with async_session() as session:
        result = await session.execute(
            select(AdminMessage).where(
                ...
                AdminMessage.is_active.is_(True),
            )
        )
        return result.scalars().all()


async def add_cycle_tip(phase: str, content: str) -> None:
    async with async_session() as session:
